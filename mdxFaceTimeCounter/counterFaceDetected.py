...
while True:
    ...
    if (len(faces) > 0):
        start = time.time()
        # print("The total time is {}".format(totalObservationTime))
        print ("Detected face. Starting counter!")
        while(len(faces) > 0):
            end = time.time()
            diff = end - start
            ...
            print("This cycle face was detected for: {}".format(elapsed))
            totalObservationTime = totalObservationTime + elapsed
            print("Total time is: {}\n".format(totalObservationTime))
            writeTotal(totalObservationTime)
